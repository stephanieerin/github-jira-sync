(function syncer() {
    ...
    var getGithubIssues = function getGithubIssues(callback) {
        ...
        _.each(context.config.github.repos, function(repo){
            getGhIssuesForRepo(repo, function(error, issues){
    ...
    var getGhIssuesForRepo = function getGhIssuesForRepo(repo, callback){
        var filter = _.extend({
            sort: 'updated',
            direction: 'desc',
            per_page: 100,
    ...
    var getClosedGithubIssues = function getClosedGithubIssues(callback) {
        ...
        _.each(context.config.github.repos, function(repo){
            getClosedGhIssuesForRepo(repo, function(issues, error){
                context.ghClosedIssues = _.union(issues, context.ghClosedIssues);
                context.ghIssues = _.union(issues, context.ghIssues);
                callback(null);
            });
        });
    ...
    var getClosedGhIssuesForRepo = function getClosedGhIssuesForRepo(repo, callback){
        var filter = _.extend({
            state: 'closed',
            sort: 'updated',
            direction: 'desc',
    ...
    var getGhIssueFor = function getGhIssue(jiraIssue) {
        _.find(context.ghIssues, function(current) {
    ...
    var createGhIssue = function createGhIssue(jiraIssue, callback) {
        context.api.jira.default.findIssue(jiraIssue.key, function getIssue(error, completeIssue){
            ...
            var repos = completeIssue.fields.components;
            // If there were no components, skip it.
            if(_.isEmpty(repos)){
                console.log('\t- Skipped because of missing component');
                callback(null);
            }
            // For each component create a github issue in the corresponding repo
            _.each(repos, function(repo){
                repo = repo.name.toLowerCase();

                if(context.config.github.repos.indexOf(repo) != -1){
                    var args = _.extend({
                    ...
                    console.log('\t- Skipped because of inaccurate component ' + repo);
                }
            });
        });
    };
    ...
    var createJiraTasksOnGithub = function createJiraTasksOnGithub(callback) {
        context.api.jira.greenhopper.getSprintIssues(context.rapidView.id, context.sprint.id, function(error, result) {
            ...
            async.each(masterIssues, addJiraSubtasks, function completed(error, completeIssue) {
